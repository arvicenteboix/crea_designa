...
def generar_documento(datos, identificativos, numero_a_letras=lambda x:str(x)):
    ...
    add_normal(parrafo_cuerpo, f"{nombre} amb NIF: {dni.lower()}\n")
...
def generar_certificas(datos, identificativos, numero_a_letras=lambda x:str(x)):
    ...
    cuerpo = (
        f"{nombre} amb nif: {dni.lower()}\n" +
        juridico + ", Designat/da per esta Subdirecció, ha impartit satisfactòriament " + cadena_movs + " del curs amb les següents dades:\n"
        + ("Codi: " + str(identificativos.get('CÓDIGO EDICIÓN / CODI EDICIÓ', '')).upper() + "\n" if identificativos else "")
        + ("Títol: " + str(identificativos.get('TÍTULO ACCIÓN FORMATIVA / TÍTOL ACCIÓ FORMATIVA', '')).upper() + "\n\n" if identificativos else "")
        + ("Data de realització: " + str("del " + identificativos.get('FECHAS REALIZACIÓN / DATES REALITZACIÓ', '')).lower() + "\n" if identificativos else "")
        + ("Lloc de realització: " + str(identificativos.get('MODALIDAD/MODALITAT', '')).upper() + "\n\n" if identificativos else "")
...
def main():
    ...
    messagebox.showwarning(
        "Aviso importante",
        "Este programa está en estado experimental y solo debe usarse en DESIGNAS para FSE. Es imprescindible revisar minuciosamente el desgina creado y corregir posibles errores, comunicándolos. Recordad que no podéis tener una misma entidad jurídica con factura y minuta. Revisad bien la ficha económica. Solo puede haber un archivo Excel en la carpeta."
    )
    ...
    chk_es_skills = tk.Checkbutton(root, text="Designa de Skills", variable=es_skills)
    ...
    # Crear un frame para contener los botones en horizontal
    button_frame = tk.Frame(root)
    button_frame.pack(expand=True, pady=10)
    
    
    # Mover los botones al frame
    btn = tk.Button(button_frame, text="Genera Designas", command=on_process("designas"), font=("Arial", 14))
    btn2 = tk.Button(button_frame, text="Genera Certificas", command=on_process("certificas"), font=("Arial", 14))
